...
const NeuralNetworkVisualizer: React.FC<NeuralNetworkVisualizerProps> = ({
  ...
  useEffect(() => {
    ...
    const inputSpacing = Math.max(minSpacing, (height - 2 * margin) / Math.max(1, network.inputNodes - 1));
    const hiddenSpacing = Math.max(minSpacing, (height - 2 * margin) / Math.max(1, network.hiddenNodes - 1));
    const outputSpacing = Math.max(minSpacing, (height - 2 * margin) / Math.max(1, network.outputNodes - 1));
    ...
    const inputLabels = ['Sensor 1', 'Sensor 2', 'Sensor 3', 'Sensor 4', 'Sensor 5', 'Speed', 'Angle Sin', 'Angle Cos'];
    ...
    for (let i = 0; i < network.inputNodes; i++) {
      ...
      // Node gradient
      const gradient = ctx.createRadialGradient(inputX, y, 0, inputX, y, nodeRadius);
    ...
    for (let i = 0; i < network.hiddenNodes; i++) {
      ...
      // Node gradient
      const gradient = ctx.createRadialGradient(hiddenX, y, 0, hiddenX, y, nodeRadius);
      gradient.addColorStop(0, `rgba(139, 92, 246, ${0.8 + hiddenActivation * 0.2})`);
      gradient.addColorStop(1, `rgba(100, 60, 200, ${0.4 + hiddenActivation * 0.6})`);
      ...
      if (isExpanded && network.biasHidden[i] !== undefined) {
        ...
        ctx.font = '8px Arial';
        ctx.fillText(`bias: ${network.biasHidden[i].toFixed(2)}`, hiddenX, y + nodeRadius + 15);
      }
    }

    // Output nodes
    const outputLabels = ['Steering', 'Acceleration'];
    for (let i = 0; i < network.outputNodes; i++) {
      ...
      // Node gradient
      const gradient = ctx.createRadialGradient(outputX, y, 0, outputX, y, nodeRadius);
      ...
      if (isExpanded) {
        ...
        if (network.biasOutput[i] !== undefined) {
          ctx.fillText(`bias: ${network.biasOutput[i].toFixed(2)}`, outputX + nodeRadius + 8, y + 15);
        }

        // Show interpretation
        const interpretation = i === 0 ?
          (activation > 0.5 ? 'Turn Right' : activation < 0.5 ? 'Turn Left' : 'Straight') :
  ...
  return (
    <div className={`bg-gray-800 rounded-lg p-4 ${className}`}>
      <h3 className="text-lg font-semibold mb-2">Neural Network Visualization</h3>
      <canvas
        ref={canvasRef}
        width={350}
        height={280}
      ...
      <div className="mt-2 text-sm text-gray-300">
        <div>Inputs: {network.inputNodes} | Hidden: {network.hiddenNodes} | Outputs: {network.outputNodes}</div>
      </div>
    </div>
  );
};
